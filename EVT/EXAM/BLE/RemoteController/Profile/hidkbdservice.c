...
static const uint8_t hidReportMap[] = {
    ...
    0x19,0x00,                  //10    LOCAL_USAGE_MINIMUM(4)
    0x29,0x77,                  //12    LOCAL_USAGE_MAXIMUM(112)
...
bStatus_t Hid_AddService(void)
{
    ...
    hidRptMap[6].id = hidReportRefClassKeyIn[0];
    hidRptMap[6].type = hidReportRefClassKeyIn[1];
    hidRptMap[6].handle = hidAttrTbl[HID_BOOT_KEY_IN_IDX].handle;
    hidRptMap[6].cccdHandle = hidAttrTbl[HID_BOOT_KEY_IN_CCCD_IDX].handle;
    hidRptMap[6].mode = HID_PROTOCOL_MODE_BOOT;
//    PRINT("%x \n",hidRptMap[7].cccdHandle);

    // Boot keyboard output report
    // Use same ID and type as LED output report
    hidRptMap[7].id = hidReportRefLedOut[0];
    hidRptMap[7].type = hidReportRefLedOut[1];
    hidRptMap[7].handle = hidAttrTbl[HID_BOOT_KEY_OUT_IDX].handle;
    hidRptMap[7].cccdHandle = 0;
    hidRptMap[7].mode = HID_PROTOCOL_MODE_BOOT;

    // Feature report
    hidRptMap[8].id = hidReportRefFeature[0];
    hidRptMap[8].type = hidReportRefFeature[1];
    hidRptMap[8].handle = hidAttrTbl[HID_FEATURE_IDX].handle;
    hidRptMap[8].cccdHandle = 0;
    hidRptMap[8].mode = HID_PROTOCOL_MODE_REPORT;

    // Battery level input report
    Batt_GetParameter(BATT_PARAM_BATT_LEVEL_IN_REPORT, &(hidRptMap[9]));
